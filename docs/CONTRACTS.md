...
```json
{
  ...
  "stub_dc": {"VDD0P8_C": 0.8},
  "care_up_to_hz": 2e10
}
```

**哪些端口建模，用户在 New 屏的 Model 列决定**，落成 `ports`。三种归宿：`model` = 表征 + 拟合 + 发射；`stub` = 引脚保留，发射成该脚直流值的理想源（轨 → 电压源，偏置 → 电流源），零仿真，报告和 `.va` 头写明 "stub, not modeled"；
**这个直流值由 `stub_dc` 给**（轨填伏特、偏置填安培）—— 约定源给的是**对偶量**（轨脚上挂的 `IL_` 是电流，
需要的是电压；偏置脚上挂的 `VB_` 是电压，需要的是电流），而 stub 按定义不跑仿真，所以这是网表唯一给不出的数。
`stub_dc` 缺这一项时，该脚**弱连而不是驱动**，并在报告里写明 —— 不编一个值；`ignore` = 无角色引脚原样接线。有角色源的引脚默认 `model`；没有角色源的引脚要先右键指定角色，工具往网表插一个约定源。Plan 只为 `model` 端口出 run，这就是最小仿真集的实际落点。

`corners` 的高级写法：复合角按 include 文件分别指定 section，例如 `{"MOSff_RCss": {"toplevel.scs": "ff", "rc.scs": "ss"}}`。简单写法 `["tt","ss"]` 时每个 include 文件的**工艺角行**统一替换（该文件确实声明了这个 section 才换）。

同一文件多行 `section=`（ADE Model Library：工艺角一行 + `pre_Sim`/`Noise_Worst` 等固定行）：只有工艺角行随角改写，固定行永远原样保留、不作为角提供、不用来播种 `corners`。工艺角行按 section 名认；认不出（没有或多于一行像工艺角）时取第一行并在 New 屏请用户选，选择写进可选键 `corner_include`：`{"toplevel.scs": 0}`（文件 → 该文件 section= 行的序号，0 起；缺省即按名字认）。
...
| 地 | 每个引脚接的地网直接从连线读：`0`、约定源的参考端（`VS_X (X AGND)` 的 `AGND`）、及用短路（L≤10 fH、R≤1 µΩ、0 V 源）连到它们的网；地网上只走到栅极/标准单元的脚是被拉低的控制脚，原样穿过 | 分地 |
...
| PMU 引脚表（`interface`） | PMU 子电路的端口，按原顺序，带每脚在台子里接的网；交付的模型按它声明端口（契约 4） |
| 相对 include | 运行网表写在各自的 run 目录里，所以相对原网表目录能找到的 `include`/`ahdl_include` 在运行网表里改成绝对路径（配方里可见，Plan 屏备注说一次）；原网表旁边找不到的裸名（ADE 的 `toplevel.scs`，仿真时靠 `-I` 找）和绝对路径不动。`spectre_ssh` 例外：跑在另一台机器上，保持相对、把目录树拷进 run 目录 |
...
规则：
- **模型和 PMU 引脚兼容：同样的脚、同样的顺序。** 模块端口 = PMU 子电路的端口表，逐个、按原顺序（New 屏引脚表的 `index`），名字也用 PMU 的（子电路内部名如 `vssa` 就叫 `vssa`）。用户在系统台子里只把 PMU 实例的 master 换成模型，**不改一根线**；按位置连接因此不会悄悄接错。
  - 建了模的脚照常：电源、轨、偏置、stub、以及被某个块当回流的地脚。
