...
4. **引脚名 ≠ 网名**：工具按 PMU 实例的**引脚**报角色，按它连到的**网**找源。两者不同名没关系，
   前缀跟着**引脚名**走（`IL_<引脚名>`）。
5. **地**：地网 = `0`，加上约定源的**参考端**（`VS_AVDD (AVDD AGND)` 里的 `AGND`），加上用短路
   （`L<=10fH`、`R<=1uΩ`、0 V vsource、iprobe）连过去的网。接在地网上的 PMU 引脚分两种：真地引脚
   （子电路里走到 MOS 源/衬底，或名字像 VSS/GND/SUB）和被台子拉低的控制脚（只走到栅极/标准单元，
   或是总线某一位）——后者原样穿过、不建模。哪条轨回哪个地，先看它的约定源参考端上有没有地引脚，
   否则从子电路内部的器件图读（就近原则），只在真地引脚里挑。如果网表里没有 PMU 的子电路定义
   （黑盒 include），地网上的引脚一律当地，工具会**明说**分不出控制脚。
6. **同一个 PDK 文件 include 了几行**（ADE Model Library 每行一条：工艺角一行，再加 `pre_Sim`、
   `Noise_Worst` 这类固定行）：只有**工艺角那一行**按角改写，其余原样保留。哪行是工艺角按 section
   名认（`tt`/`ss`/`ff`/`sf`/`fs` 作为 `_` 分隔的一个词，或 `typical`/`rcworst` 之类）；认不出来就用
   第一行并在 New 屏请你选，选择存进配置的 `corner_include`。
