...
| M5 | `runner.py` + `importer.py`（老仓 `import_cadence.py` 接新契约：读 ADE 结果目录的 `input.scs` + psf → 认角/温度/VSET/激励 → 填数据集，台账 `imported`；对不上的列出；CSV 需用户映射）：一个接口，后端 `spectre_ssh`（真，经 VM）、`dry_run`、`fake`、`donau_alps`（搬 `cluster/`，dry-run）；按 run_id 缓存 resume；写台账；PSF 读取搬 `binpsf.py`/`psf.py` | PMU_DEMO 全计划经 spectre_ssh 跑完落成契约 2 数据集；重跑全部 `skipped_cached`；台账每条有 `consumes`；**把已跑完的 psf 目录当外部结果导入一个新项目，计划里对应 run 显示 `imported`，只剩缺的要跑** |
...
| New | `POST /api/p/<n>/netlist`（上传、路径或 reread）· `GET /api/p/<n>/netlist`（来源路径、候选实例、上次读入的变化）· `PUT /api/p/<n>/netlist/instance`（换 PMU 实例）· `POST /api/p/<n>/import`（结果目录列表 → 匹配报告：填了哪些格子、哪些对不上）· `GET /api/p/<n>/pins` · `PUT /api/p/<n>/pins/<pin>`（role/fate）· `PUT /api/p/<n>/pins`（批量 fate：全部 rail / 全部 bias / 全不，一次撤销）· `GET/PUT /api/p/<n>/config` · `GET /api/p/<n>/config/derived` · `POST /api/p/<n>/config/undo` · `POST /api/p/<n>/measure-load` | 解析网表、引脚表、model/stub/ignore、三问、推导配置、撤销、从网表量负载 |
...
| Run | `GET /api/p/<n>/ledger?status=` · `GET /api/p/<n>/runs/<id>` · `GET /api/p/<n>/runs/<id>/log`（流式）· `GET /api/p/<n>/runs/<id>/bundle` · `POST /api/p/<n>/runs/<id>/{retry,skip,kill}` · `POST /api/p/<n>/fit` | 台账、详情、日志流、失败包、动作、拟合 |
