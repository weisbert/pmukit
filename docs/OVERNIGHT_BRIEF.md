...
| New | `POST /api/p/<n>/netlist`（上传、路径或 reread）· `GET /api/p/<n>/netlist`（来源路径、候选实例、上次读入的变化）· `PUT /api/p/<n>/netlist/instance`（换 PMU 实例）· `POST /api/p/<n>/import`（结果目录列表 → 匹配报告：填了哪些格子、哪些对不上）· `GET /api/p/<n>/pins` · `PUT /api/p/<n>/pins/<pin>`（role/fate）· `GET/PUT /api/p/<n>/config` · `GET /api/p/<n>/config/derived` · `POST /api/p/<n>/config/undo` · `POST /api/p/<n>/measure-load` | 解析网表、引脚表、model/stub/ignore、三问、推导配置、撤销、从网表量负载 |
