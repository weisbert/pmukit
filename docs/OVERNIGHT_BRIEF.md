...
> 给一个全新的 ultracode 会话，**在本机的 Linux VM 上运行**（Spectre 18.1 在那里）。主控代理先读完本文、`REFACTOR_PLAN.md`、`CONTRACTS.md`、`UX_RULES.md`、老仓的 `docs/reference/METHODOLOGY.md` 和 `TOOL_FACTS.md`，再动手。
...
- Spectre 18.1：`/home/yusheng/Program/eda/cadence/SPECTRE181/bin/spectre`，环境见老仓 `cadence/spectre_run.py::_env()`，编译 `.va` 必须 `-64`，在 scratch 目录编译副本。
- 老仓 checkout：`~/cadence_work/Test/workarea/LDO_modeling`（已 private，只读参考；若路径不同，`find ~ -maxdepth 4 -name LDO_modeling -type d`）。
- ALPS / Donau / dsub **不在 VM 上**：这两个后端只能 dry-run，代码从老仓 `cadence/cluster/` 原样搬（那是盒子上验证过的），明天在盒子上首跑。
- 黑名单：`python tools/make_denylist.py --manifest <老仓>/cadence/insitu/manifests/REAL_*.json --extra <项目号等>` 生成 `.pmukit-denylist`（git-ignored）。开工第一步。
- Python 3.11 venv，`requirements.txt`（numpy、scipy、pytest）。node 用于 web 壳语法检查（没有就跳过该检查并记录）。
