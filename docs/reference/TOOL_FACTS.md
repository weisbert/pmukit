...
- **Simulation dirs live in `$WORK_ROOT/pmukit/<project>/runs/<run_id>`**, apart from the tool and the data, exactly as LDO_modeling ran its validated sweep in `$WORK_ROOT/ldo_modeling/...` (the tree ADE's own Donau jobs write to). `$PMUKIT_SIM_ROOT` overrides; with neither set (the desk) runs stay beside the project data.
...
- **Install PREFIX** = `<site-data-root>/<project-area>/<employee-id>/workarea/LDO_modeling`; update via `bash apply` (auto-detects incremental/full). `~/.ldo_modeler/` is the GUI config dir (distinct). skillbridge==1.8.0 in `.venv`.
