      ...
      <div class="pb">
        ...
        <sc-if value="{{{{ loaded }}}}" hint-placeholder-val="{{{{ false }}}}">
          <div class="kv" style="margin-bottom:12px">
            ...
            <span class="k">PMU instance</span><span><span class="mono">PMU_TOP</span> · cell <span class="mono">PMU_DEMO</span> · 11 pins · <b>{{{{ portSummary }}}}</b></span>
          ...
          <table class="t">
            <thead><tr><th style="width:26px" title="Model this port? Unticked rails/biases become stubs: the pin stays, an ideal source at its DC value, no simulations.">Model</th><th>Pin</th><th>Net</th><th>Role</th><th>From source</th><th>DC</th><th>Fate</th></tr></thead>
            <tbody>
              <sc-for list="{{{{ pins }}}}" as="p" hint-placeholder-count="6">
                <tr class="rc" onContextMenu="{{{{ p.menu }}}}"><td><sc-if value="{{{{ p.canModel }}}}" hint-placeholder-val="{{{{ true }}}}"><input type="checkbox" checked="{{{{ p.isModel }}}}" onChange="{{{{ p.toggleModel }}}}"></sc-if></td><td class="mono">{{{{ p.pin }}}}</td><td class="mono" style="color:#5d5a53">{{{{ p.net }}}}</td><td><span class="badge {{{{ p.roleCls }}}}">{{{{ p.role }}}}</span></td><td class="mono">{{{{ p.src }}}}</td><td class="num" style="text-align:left">{{{{ p.dc }}}}</td><td><span class="badge {{{{ p.fateCls }}}}">{{{{ p.fateText }}}}</span></td></tr>
              </sc-for>
            </tbody>
          </table>
          <div class="callout" style="margin-top:12px;display:flex;gap:8px;align-items:flex-start"><span style="color:#1f6f9f;margin-top:2px">{ICONS['check']}</span><div><b>You decide which ports get modeled.</b> Tick = characterized and fitted. Untick a rail or bias you do not use = <b>stub</b>: the pin stays on the symbol, emitted as an ideal source at its DC value, zero runs. VDD0P8_C is a stub here because this block never draws from it.</div></div>
          <div class="callout warn" style="margin-top:8px;display:flex;gap:8px;align-items:flex-start"><span style="color:#b7791f;margin-top:2px">{ICONS['alert']}</span><div><b>TESTMODE</b> has no IL_/VB_/VS_/VEN_ source, so it gets no role and is left exactly as wired. If it is a rail or bias, right-click → set role; pmukit inserts the named source for you.</div></div>
        </sc-if>
      </div>
    </div>
  </div>
    ...
    <div class="panel" style="flex:1">
      ...
      <div class="pb" style="display:flex;flex-direction:column;gap:18px">
        ...
        <div class="field"><span class="lbl">2 · What your block draws from each rail<span class="q" title="On/off currents set the characterization grid (off, 0.2×on, on, 2×on). Switches = your block turns on and off during simulation; that load step is the one large-signal event modeled.">?</span></span>
          ...
          <span class="hint">Only ports ticked "Model" appear here. On/Off defaults come from the IL_ source's dc. "Switches" adds the load-EN on/off transient, the one large-signal event that is modeled.</span>
        </div>
...
class Component extends CtxLogic {
  constructor(p){ super(p); this.state = { loaded: false, adv: false, fate: { VDD0P8_C: 'stub' },
  ...
  renderVals(){
    ...
    ].map(p => { const fate = s.fate[p.pin] || (p.role === 'none' ? 'ignore' : p.role === 'ground' || p.role === 'supply' || p.role === 'enable' ? 'wired' : 'model');
      const setFate = f => () => { const o = Object.assign({}, s.fate); o[p.pin] = f; this.setState({fate: o}); };
      return { ...p, fate, fateCls: fate === 'model' ? 'b-acc' : fate === 'stub' ? 'b-ink' : 'b-mute', fateText: fate === 'model' ? 'model' : fate === 'stub' ? 'stub · ideal ' + (p.role === 'rail' ? 'V' : 'I') : fate === 'wired' ? 'wired' : 'ignore',
      canModel: p.role === 'rail' || p.role === 'bias', isModel: fate === 'model', toggleModel: () => setFate(fate === 'model' ? 'stub' : 'model')(),
      menu: (e) => this.openMenu(e, 'pin ' + p.pin, [
      { label: 'Model this port', disabled: !(p.role === 'rail' || p.role === 'bias'), action: setFate('model') }, { label: 'Stub: keep the pin, ideal source at its DC value, no runs', disabled: !(p.role === 'rail' || p.role === 'bias'), action: setFate('stub') }, { label: 'Ignore (leave wired as-is)', action: setFate('ignore') }, '-',
    ...
    const nR = pins.filter(p => p.role === 'rail' && p.fate === 'model').length, nB = pins.filter(p => p.role === 'bias' && p.fate === 'model').length, nS = pins.filter(p => p.fate === 'stub').length;
    const portSummary = nR + ' rails · ' + nB + ' biases modeled · ' + nS + ' stub';
    const stubs = pins.filter(p => p.fate === 'stub').map(p => p.pin).join(',');
    return { ...this.menuVals(),
      ...this.helpVals('New project', ['Bring one Spectre netlist exported at the nominal corner. Sources named IL_ / VB_ / VS_ / VEN_ tell pmukit which pin is which; nothing else is needed.', 'You choose which ports get modeled (the Model column). Unticked rails and biases become stubs: pin kept, ideal source at its DC value, no runs. That is the minimal simulation set.', 'Answer three things only you know. Everything under "derived settings" is decided by pmukit and can be overridden later.', 'Nothing runs yet. Build plan shows what would run and what it costs before anything is submitted.'], [{k:'Ctrl O', d:'choose a netlist file'}, {k:'Ctrl Enter', d:'Build plan'}, {k:'A', d:'show / hide derived settings'}]),
      ...this.cliVals(s.loaded ? `pmukit new demo_pmu --netlist tb/input.scs --pmu PMU_TOP --corners ${onC} --temps ${onT} --vset 3 --load VDD0P8_A=500u/2u${s.sw.VDD0P8_A ? '/switch' : ''} --load VDD0P8_B=2m/20u${s.sw.VDD0P8_B ? '/switch' : ''}${stubs ? ' --stub ' + stubs : ''} --fmax 20G` : 'pmukit new demo_pmu --netlist <input.scs>'),
      portSummary, loaded: s.loaded, notLoaded: !s.loaded, adv: s.adv,
      ...
      footText: s.loaded ? `${nc} corners × ${nt} temps × 1 VSET = ${nc*nt} cells · ${portSummary} · EN` : 'Load a netlist to continue',
